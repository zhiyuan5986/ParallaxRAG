...
def eval_all(pred_file_path, run, subset, split=None, eval_hops=-1):
    ...
    if subset:
        ...
        postfix = ""
    swanlab.log({f"results{postfix}/hit@1": hit1,
...
def main():
    ...
    parser.add_argument("-d", "--dataset_name", type=str, default="cwq", 
    ...
    parser.add_argument("--max_model_len", type=int, default=8192*2, help="Max model length (None = use model default)")
    ...
    parser.add_argument("--max_num_seqs", type=int, default=128, help="Max number of sequences to process in parallel")
    ...
    llm, client = llm_init(model_name, tensor_parallel_size, max_model_len, max_tokens, seed, temperature, frequency_penalty, gpu_memory_utilization, max_num_seqs)
    ...
    with open(raw_pred_file_path, "a") as pred_file:
        ...
        for idx, each_qa in enumerate(tqdm(data[start_idx:], initial=start_idx, total=len(data))):
            try:
                res, usage_stats = llm_inf_all(llm, each_qa, llm_mode, model_name)
            except Exception as e:
                print(f"[Skip] Error processing id={each_qa.get('id')} — {e}")
                skipped_cnt += 1
                continue  # Skip writing this sample; it won't be evaluated later.

            # Drop large fields to save memory.
            ...
            # Run periodic evaluation every 500 samples using the -resume file.
            processed_count = start_idx + idx + 1
            if processed_count % 500 == 0:
                try:
                    ...
                    pass
                # The eval script writes files by replacing 'predictions.jsonl';
                # provide a path ending with that name to avoid overwriting -resume.
                tmp_eval_file_path = raw_pred_file_path.with_name(raw_pred_file_path.name.replace('-resume', ''))
                try:
                    if os.path.exists(tmp_eval_file_path):
                        os.remove(tmp_eval_file_path)
                    shutil.copyfile(raw_pred_file_path, tmp_eval_file_path)
                    print(f"Performing periodic evaluation at {processed_count} samples...")
                    # Use a string path to avoid downstream replace errors.
                    tmp_eval_file_str = str(tmp_eval_file_path)
                    try:
                        eval_all(tmp_eval_file_str, run, subset=True)
                        eval_all(tmp_eval_file_str, run, subset=False)
                    except Exception as e:
                        print(f"[Periodic Eval Skip] Error during evaluation at {processed_count}: {e}")
                finally:
                    try:
                        if os.path.exists(tmp_eval_file_path):
                            os.remove(tmp_eval_file_path)
        ...
        print(f"Finished inference with {skipped_cnt} samples skipped due to errors.")
    ...
    # Final evaluation.
    #final_pred_file_str = str(final_pred_file_path)
    final_pred_file_str = str(raw_pred_file_path)
    eval_all(final_pred_file_str, run, subset=True)
    eval_all(final_pred_file_str, run, subset=False)
